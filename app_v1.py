...
def ask():
    ...
    try:
        ...
        # ========== DUAL-LLM DECISION: Decide whether to generate an infographic ==========
        logger.info("🤖 [DUAL-LLM] Deciding whether to generate an infographic...")
        decision = ai_services.decide_make_infographic(raw_text, original_question=question)
        
        result = {'response': raw_text}

        if decision.get('make'):
            logger.info(f"✅ Decision: MAKE infographic. Reason: {decision.get('reason')}")
            try:
                ...
                if image_path:
                    ...
                    logger.error("❌ [LLM 2] Infographic generation failed.")
            except Exception as e:
                logger.error(f"❌ [LLM 2] Exception during infographic generation: {e}")
        else:
            logger.info("✅ Decision: DO NOT make infographic. Returning text only.")
